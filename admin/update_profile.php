...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (isset($_POST['update'])) {
    ...
    if ($ppass !== $prev_pass) {
      ...
      $insert_updates = $conn->prepare('UPDATE `admins` SET name = ? , password = ? WHERE id = ?');
      $insert_updates->execute([$name, $npass, $_SESSION['admin_id']]);
          ...
          <div class="card-body">
            <form method="POST">
              <div class="mb-3">
                ...
                <input type="hidden" value="<?= $fetch_profile['password'] ?>" name="prev_pass">
                <input type="text" class="form-control" value='<?= $fetch_profile['name']; ?>' name="name" id="name"
